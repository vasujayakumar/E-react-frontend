...
const FloatingChatWindow = () => {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                // 获取当前用户的ID和身份
                const response = await axios.get('http://e-react-node-backend-22ed6864d5f3.herokuapp.com:8080/api/chat/getCurrentId');
                ...
                if (response.data.identity === "doctor") {
                    userListResponse = await axios.get(`http://e-react-node-backend-22ed6864d5f3.herokuapp.com:8080/api/chat/getDoctorChatList?doctorId=${C_ID}`);
                } else if (response.data.identity === "patient") {
                    userListResponse = await axios.get(`http://e-react-node-backend-22ed6864d5f3.herokuapp.com:8080/api/chat/getPatientChatList?patientId=${C_ID}`);
                }
        ...
        ws.current = new WebSocket('ws://e-react-node-backend-22ed6864d5f3.herokuapp.com:8080/api/chat/sendMessage');
    ...
    const handleUserClick = (userId) => {
        ...
        axios.get('http://e-react-node-backend-22ed6864d5f3.herokuapp.com:8080/api/chat/getConversationIdByUserIdentity', {
            params: {
                sender: currentId,
            ...
            .then(response => {
                ...
                if (conversationId) {
                    ...
                    let CH_history = axios.get(`http://e-react-node-backend-22ed6864d5f3.herokuapp.com:8080/api/chat/getChatHistoryByConversationId?conversationId=${conversationId}`);
